...
登录后可以写文章、发布、点赞、收藏、评论，覆盖主链路。种子数据包含 24 篇文章、10 个标签、6 个用户，以及 4 位讲师、10 门公益免费课程和 30 条报名记录。

课程相关的验收入口：首页「精品课程」板块（分类 tab + 「加载更多」）、课程详情 `http://localhost:3000/courses/1`，后台在右上角头像下拉里进「课程管理」`http://localhost:3000/admin/courses` 与「讲师管理」`http://localhost:3000/admin/teachers`（需管理员账号）。课程管理页支持按关键词搜索（标题 / 讲师 / 标签）与状态、难度筛选，列表 24 条一页、滚到底自动加载更多。
